...
void MakeCentralityGraphs_alice_ref(TString sInputName = "toyFlow.root", TString sOutputName = "toyFlowGraphs.root", const int iDet=6, const int iVnDet=6) {
    ...
    double inputFlow[nCoef][CENTBINS_N-4] = {
        ...
        {0.02446622, 0.03929768, 0.05729937, 0.07382241, 0.08401309, 0.08807433, 0.0},
    ...
    //=====vn=====
    // Observed vn
    TH1D *hVnObs[nCoef][CENTBINS_N];
    for (i=0; i<nCoef; i++) {
        for (j=0; j<CENTBINS_N-4; j++) {
            hVnObs[i][j] = (TH1D*)fIn->Get(Form("toyflow/hVnObs/hVnObsH%02iD%02iCENT%02i",i+1,iVnDet,j));
            checkUnderOverFlow(hVnObs[i][j]);
        }
    }
    ...
    for (i=0; i<nCoef; i++) {
        ...
        for (j=0; j<CENTBINS_N-4; j++) {
            cout << "cent=" << j << "\n";
            n = i+1;
            vn[i][j] = hVnObs[i][j]->GetMean();
            vnTrue[i][j] = hVnObs[i][j]->GetMean();
            ...
            vn[i][j] /= R[i][j];
            errorVn[i][j] = GetVnError(hVnObs[i][j]->GetMean(), hVnObs[i][j]->GetMeanError(), R[i][j], errorR[i][j]);

            vnTrue[i][j] /= hRtrue[i][iDet][j]->GetMean();
            errorVnTrue[i][j] = GetVnError(hVnObs[i][j]->GetMean(), hVnObs[i][j]->GetMeanError(), hRtrue[i][iDet][j]->GetMean(), hRtrue[i][iDet][j]->GetMeanError());
        }
    }
    ...
    for(i=0; i<nCoef; i++){
        ...
        gVnObs[i] = new TGraphErrors(CENTBINS_N-4);
        ...
        for (j=0; j<CENTBINS_N-4; j++) {
            ...
            gVnObs[i]->SetPoint(j, (centBins[j]+centBins[j+1])/2-0.5, hVnObs[i][j]->GetMean());
            gVnObs[i]->SetPointError(j, 0.0, hVnObs[i][j]->GetMeanError());
        }
    }
    ...
    for (i=0; i<nCoef; i++) {
        ...
        gVnObs[i]->Write(Form("gVnObsH%02i", i+1));
    }
