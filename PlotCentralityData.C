...
void PlotCentralityData(int readNFiles = 1) {
    ...
    for(int iFil=0; iFil<readNFiles; iFil++) {
        for (i=0; i<nCoef; i++) {
            ...
            gR[i][iFil]->SetMarkerColor(1+iFil);
            gR[i][iFil]->SetMarkerSize(mSize);
            gR[i][iFil]->SetFillColor(1+iFil);

            gRtrue[i][iFil]->SetMarkerStyle(mMarker+1);
            gRtrue[i][iFil]->SetMarkerColor(1+iFil);
            gRtrue[i][iFil]->SetMarkerSize(mSize);
            gRtrue[i][iFil]->SetFillColor(1+iFil);

            gVn[i][iFil]->SetMarkerStyle(mMarker);
            gVn[i][iFil]->SetMarkerColor(1+iFil);
            gVn[i][iFil]->SetMarkerSize(mSize);
            gVn[i][iFil]->SetFillColor(1+iFil);

            gVnTrue[i][iFil]->SetMarkerStyle(mMarker+1);
            gVnTrue[i][iFil]->SetMarkerColor(1+iFil);
            gVnTrue[i][iFil]->SetMarkerSize(mSize);
            gVnTrue[i][iFil]->SetFillColor(1+iFil);

            gVnEP[i][iFil]->SetMarkerStyle(mMarker+3);
            gVnEP[i][iFil]->SetMarkerColor(1+iFil);
            gVnEP[i][iFil]->SetMarkerSize(mSize);
            gVnEP[i][iFil]->SetFillColor(1+iFil);

            gVnSP[i][iFil]->SetMarkerStyle(mMarker+4);
            gVnSP[i][iFil]->SetMarkerColor(1+iFil);
            gVnSP[i][iFil]->SetMarkerSize(mSize);
            gVnSP[i][iFil]->SetFillColor(1+iFil);
        }
    }

    gStyle->SetOptStat(0);

    TCanvas *c1 = new TCanvas("c1", "c1", nXP[nHarmonics], nYP[nHarmonics]);
    ...
    for (i=1; i<nHarmonics+1; i++) {
        for(int iFil=0; iFil<readNFiles; iFil++) {
            ...
            gVn[i][iFil]->Draw(Form("%s",sSame[iFil].Data()));
    ...
    TLegend *leg1 = new TLegend(0.30,0.5,0.55,0.85,"","brNDC");
    ...
    if(readNFiles>1) leg1->AddEntry(gR[0][1], "8 sec granular detector", "f");
    if(readNFiles>2) leg1->AddEntry(gR[0][2], "16 sec granular detector", "f");
    leg1->Draw("SAME");
    c1->Draw();
    ...
    leg2->AddEntry(gRtrue[0][nRef], "R true, uniform #phi", "p");
    leg2->AddEntry(gR[0][nRef], "R sub event method, uniform #phi", "p");
    if(readNFiles>1) leg2->AddEntry(gR[0][0], "Ideal detector", "f");
    if(readNFiles>1) leg2->AddEntry(gR[0][1], "8 sec granular detector", "f");
    if(readNFiles>2) leg2->AddEntry(gR[0][2], "16 sec granular detector", "f");
    leg2->Draw("SAME");
    c2->Draw();
